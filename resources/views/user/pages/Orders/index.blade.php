                                            ...
                                            <li class="list-group-item py-3 py-lg-0 px-0 border-top">
                                                <div class="row align-items-center">
                                                    ...
                                                    <div class="col-4 col-md-3 col-lg-2 mx-auto">
                                                        <form method='POST'
                                                            action="{{ route('cartadd', $cart->id_cart) }}">
                                                            @csrf
                                                            <div class="input-group input-spinner flex-nowrap w-100">
                                                                <a href='{{ route('minus', [$cart->id_cart]) }}'
                                                                    class='text-decoration-none btn btn-sm'>
                                                                    <i class="bi bi-dash-lg"></i>
                                                                </a>
                                                                <input type="text" value="{{ $cart->amount }}"
                                                                    name="cart_quant"
                                                                    class="quantity-field form-control form-input ">
                                                                <a href='{{ route('addmore', [$cart->id_cart]) }}'
                                                                    class='text-decoration-none btn btn-sm'>
                                                    ...
                                                    <div class="col-2 text-lg-end text-start text-md-end col-md-2">
                                                        <span
                                                            class="fw-bold">${{ $cart->Product->sale > 0 ? $cart->Product->price * (1 - $cart->Product->sale / 100) : $cart->Product->price }}
                                                            /kg</span>
                                                    </div>
                                                </div>
                                                @php
                                        ...
                                        @foreach ($carts as $key => $cart)
                                            <li class="list-group-item py-3 py-lg-0 px-0 border-top">
                                                <div class="row align-items-center">
                                                    ...
                                                    <div class="col-2 text-lg-end text-start text-md-end col-md-2">
                                                        <span
                                                            class="fw-bold">${{ $cart['sale'] > 0 ? $cart['per_price'] * (1 - $cart['sale'] / 100) : $cart['per_price'] }}
                                                            /kg</span>
                                                    </div>
                                                </div>
                                            </li>
                                    ...
                                    <ul class="list-group list-group-flush" id="summary">
                                        ...
                                        <li id="added_coupon"
                                            ...
                                            <div class="ms-auto text-danger">
                                                <span
                                                    id="discount">{{ $coupon ? ($coupon->freeship ? '- $' . number_format($coupon->discount, 2, '.', ' ') : '- ' . $coupon->discount . '%') : '' }}</span>
                                            </div>
                                        </li>
                                        <li class="list-group-item d-flex justify-content-between align-items-start">
                                            <div class="me-auto">
                                            ...
                                            <span class="fw-bold" id="total_items"
                                                data-total="{{ $sum }}">${{ $coupon == null ? number_format($sum, 2, ',', ' ') : ($coupon->freeship ? number_format($sum - $coupon->discount, 2, '.', ' ') : number_format($sum * (1 - $coupon->discount / 100), 2, '.', ' ')) }}</span>
                                        </li>
                                    </ul>
                                </div>
                                <div class="d-grid mb-1 mt-4">
                                    <a href="{{ route('checkout') }}"
                                        ...
                                        to Checkout <span class="fw-bold"
                                            id="total_cart">${{ $coupon == null ? number_format($sum, 2, ',', ' ') : ($coupon->freeship ? number_format($sum - $coupon->discount, 2, '.', ' ') : number_format($sum * (1 - $coupon->discount / 100), 2, '.', ' ')) }}</span></a>
                                </div>
        ...
        $(document).ready(function() {
            $('#checkCoupon').click(function() {
                if ($('#giftcard').val().length > 0) {
                    $.get(window.location.origin + "/public/index.php/ajax/add-coupon/" + $('#giftcard')
                        .val(),
                        function(data) {
                            // console.log(data);

    ...
    <script>
      $(document).ready(function(){
            $('.remove_add').click(function(){
              window.location.assign(window.location.origin+'/ProjectSem2-Quan/public/remove_address/'+$(this).data('idadd'));
            });
            ...
            $("#paypal_btn").click(function(){
              @if(Auth::check())
                $.get(window.location.origin+"/ProjectSem2-Quan/public/ajax/get-address/"+$('input[name=select_address]:checked').data('address'), function(data){
                  ...
                  $("#confirm_paypal").click(function(){
                    window.location.assign(window.location.origin+'/ProjectSem2-Quan/public/process-transaction?select_address='+$('input[name=select_address]:checked').val()+"&instruction="+$("#DeliveryInstructions").val()+"&shipfee="+$('input[name=shipment_fee]').val()+"&coupon="+$('input[name=code_coupon]').val());
                  });
                })
              @else

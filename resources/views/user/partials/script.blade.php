<script>
    $(document).ready(function() {
            ...
            $('.btn_showcart').click(function() {
                ...
                data) {
                    ...
                    $('input[name=quan]').on('focusout', function(e) {
                        ...
                        console.log($(this).val() == parseInt($(this).parent().data(
                            'amount')));
                        console.log(parseInt($(this).parent().data('amount')));
                        if (parseInt($(this).parent().data('amount')) != $(this)
                        .val()) {
                            $(this).parent().next().removeClass('d-none');
                        } else {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.data.forEach(el => {
                    $("#province").append(
                        `<option value="${el.ProvinceID}">${el.ProvinceName}</option>`)
                })
            },
            ...
            $.ajax({
                ...
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('Token',
                        '40c06a9e-ee0f-11ed-a281-3aa62a37e0a5');
                    xhr.setRequestHeader("Content-Type", "application/json")
                },
        ...
        $('#ward').change(function() {
            ...
            @endif
            $.get(window.location.origin + '/ProjectSem2/public/ajax/ghtk_service/fee?province=' + $(
                "#province option:selected").text() + "&district=" + $(
                "#district option:selected").text(), function(data6) {
                    ...
                    if (parseInt($('#delivery_method option:selected').val()) < 10) {
                        ...
                        $.get(window.location.origin +
                            ...
                            function(data1) {
                                ...
                                let deliver_method4 = jQuery.parseJSON(dataJson3[$(
                                        '#delivery_method option:selected')
                                    .val()]);
                                if (deliver_method4['fee']['delivery']) {
                                    let totall3 = parseInt($("#total").data(
                                        'subtotal')) + deliver_method4['fee'][
                                        'fee'
                                    ];
                                    if (deliver_method4['fee']['fee'] != $(
                                            "input[name=shipment_fee]").val()) {
                                        $("#shippment_fee").html(deliver_method4[
                                            'fee']['ship_fee_only'] + " đ");
                                        $("#total").html(totall3 + " đ");
                                    }
                                    $(".totalPay").text((totall3 * 0.000043)
                                        .toFixed(2));
                                    $("input[name=shipment_fee]").val(
                                        deliver_method4['fee']['fee']);
                                    if (parseInt($('#delivery_method').val()) ==
                                        2) {
                                        $('#extra_ship').parent().addClass(
                                        'd-none');
                                    } else {
                                        $('#extra_ship').parent().removeClass(
                                            'd-none');
                                    }
                                    if (deliver_method4['fee']['extFees'].length >
                                        0) {
                                        let transtalate2 = {
                                            "Phụ phí hàng nông sản/thực phẩm khô": "Surcharge for agricultural products/dry food"
                                        };
                                        let ex_fee3 = 0;
                                        deliver_method4['fee']['extFees'].forEach(
                                            el => {
                                                ex_fee3 += el['amount'];
                                                $('#extra_ship').html(
                                                    `<div class='ms-3 text-muted'>${transtalate2[el['title']]}</div>`
                        ...
                        $.get(window.location.origin +
                            "/ProjectSem2/public/ajax/ghn_service/fee?ward=" + $(
                                '#ward option:selected').val() + "&district=" + $(
                                '#district option:selected').val() +
                            "&service_id=" + $('#delivery_method option:selected')
                            .val(),
                            function(data5) {
                                let newdata4 = data5.slice(0, data5.length - 1);
                                let dataJs4 = jQuery.parseJSON(newdata4);
                                if (dataJs4['code'] == 200) {
                                    let total_ghn = dataJs4['data']['total'] +
                                        parseInt($("#total").data('subtotal'));
                                    let shipping = dataJs4['data']['total'];
                                    if (shipping != $("input[name=shipment_fee]")
                                        .val()) {
                                        $("#shippment_fee").html(shipping + " đ");
                                        $("#total").html(total_ghn + " đ");
                                    }
                                    $(".totalPay").text((total_ghn * 0.000043)
                                        .toFixed(2));
            ...
            $.get(window.location.origin + "/ProjectSem2/public/ajax/ghn_service/service?district=" + $(
                '#district option:selected').val(), function(data) {
                ...
                dataJs['data'].forEach(service => {
                    ...
                    str2 +=
                        `<option value='${service['service_id']}'>${translate}</option>`;
                    $.get(window.location.origin +
                        "/ProjectSem2/public/ajax/ghn_service/fee?ward=" + $(
                            '#ward option:selected').val() + "&district=" + $(
                            '#district option:selected').val() + "&service_id=" +
                        service['service_id'],
                        function(data2) {
                            ...
                            $('#delivery_method').change(function() {
                                if (parseInt($(
                                    ...
                                    .val() + "&district=" + $(
                                        ...
                                        function(data4) {
                                            let newdata3 = data4.slice(
                                                0, data4.length - 1);
                                            let dataJs3 = jQuery
                                                .parseJSON(newdata3);
                                            let total_ghn = dataJs3[
                                                    'data']['total'] +
                                                parseInt($("#total")
                                                    .data('subtotal'));
                                            let shipping = dataJs3[
                                                'data']['total'];
                                            if (shipping != $(
                                                    "input[name=shipment_fee]"
                                                    ).val()) {
        ...
        $("#submit_order").click(function() {
            $('#delivery_method option:selected').val($('#delivery_method option:selected').parent()
                .attr('label') + " - " + $('#delivery_method option:selected').text());
            @if (!Auth::check())
                $('#province option:selected').val($('#province option:selected').text());
        ...
        $('.user_editorder').click(function() {
            $.get(window.location.origin + "/ProjectSem2/public/account/ajax/edit_order/" + $(this)
                ...
                function(data) {
                    ...
                    const checkChanged = () => {
                        if ($("#edit_cusname").val().trim().length > 0 && $("#edit_cusphone")
                            .val().trim().length > 0 && $("#edit_cusemail").val().length > 0 &&
                            $("#edit_ward").val().length > 0 && $("#edit_province").val()
                            .length > 0) {
                            return true;
                        } else {
                            return false;
                                    ...
                                    success: function(data) {
                                        ...
                                        data.data.forEach(el => {
                                            if (add[add
                                                ...
                                                list_province
                                                    +=
                                                    `<option value="${el.ProvinceID}" selected>${el.ProvinceName}</option>`;
                                                selected_province
                                                    = el
                                                    .ProvinceID;
                                            } else {
                                    ...
                                    success: function(data) {
                                        ...
                                        data.data.forEach(value => {
                                            if (add[add
                                                ...
                                                list_district
                                                    +=
                                                    `<option value="${value.DistrictID}" selected>${value.DistrictName}</option>`;
                                                seleted_district
                                                    = value
                                                    .DistrictID;
                                            } else {
                                    ...
                                    success: function(data) {
                                        ...
                                        data.data.forEach(value => {
                                            if (add[add
                                                ...
                                                list_ward +=
                                                    `<option value="${value.WardCode}" selected>${value.WardName}</option>`;
                                            } else {
                                                list_ward +=
                                                    `<option value="${value.WardCode}">${value.WardName}</option>`;
                                            }
                        ...
                        $("#edit_ward").change(function() {
                            ...
                            if (logictic[0] == "Giao Hang Tiet Kiem") {
                                ...
                                $.get(window.location.origin +
                                    ...
                                    "&district=" + $("#edit_district option:selected")
                                    .text() + "&method=" + method,
                                    function(ghtk_data) {
                                        let dataJson = jQuery.parseJSON(ghtk_data);
                                        let deliver_method = jQuery.parseJSON(dataJson[
                                            0]);
                                        if (deliver_method['fee']['delivery']) {
                                            $("#edit_servicefee").parent().parent()
                                                .next().removeClass('d-none');
                                            $('#edit_servicefee').parent().parent()
                                                .next().next().removeClass('d-none');
                                            $("#new_servicefee").val(deliver_method[
                                                'fee']['fee'] + " đ");
                                        } else {
                                            $("#submit_editorder").attr('disabled',
                                                'disabled');
                                            $("#error_delivery").html(
                                                'Look like Giao Tiet Kiem didn\'t support this location, Try to choose another province or district or ward near you'
                                                );
                                        };
                                    });
                            } else {
                                ...
                                $.get(window.location.origin +
                                    "/ProjectSem2/public/ajax/ghn_service/service?district=" +
                                    $('#edit_district option:selected').val(),
                                    function(ghn_data) {
                                        let newdata = ghn_data.slice(0, ghn_data
                                            .length - 1);
                                        let dataJs = jQuery.parseJSON(newdata);
                                        if (dataJs['code'] == 400) {
                                            $("#submit_editorder").attr('disabled',
                                                'disabled');
                                            $("#error_delivery").html(
                                                'Look like Giao Hang Nhanh didn\'t support this location, Try to choose another province or district or ward near you'
                                                );
                                            ...
                                            dataJs['data'].forEach(ser => {
                                                if (ser['short_name'] ==
                                                    ...
                                                    $.get(window.location
                                                        ...
                                                        "/ProjectSem2/public/ajax/ghn_service/fee?ward=" +
                                                        $(
                                                            '#edit_ward option:selected')
                                                        .val() +
                                                        "&district=" + $(
                                                            '#edit_district option:selected'
                                                            ).val() +
                                                        "&service_id=" +
                                                        ser['service_id'] +
                                                        "&weight=" +
                    ...
                    $("#close_editorder").click(function() {
                        ...
                        $('#edit_servicefee').parent().parent().next().next().addClass(
                            'd-none');
                    })
                })
        });
        ...
        $("input[name=new_password2]").change(function() {
            if (!valPass.test($(this).val())) {
                ...
                $('#msg_register_password').text(
                    "Password is invalid. >= 8 characters, at least 1 normal, at least 1 number)");
            } else if ($("input[name=reenter_password]").val().trim().length > 0 && $(this).val() != $(
                    "input[name=reenter_password]").val()) {
                if ($(this).hasClass('is-valid')) {
                    $(this).removeClass('is-valid');
                }
                $(this).addClass('is-invalid');
            ...
            if ($("input[name=reenter_password]").hasClass('is-valid') && $("input[name=new_password]")
                .hasClass('is-valid') && $("input[name=reenter_password]").val() == $(
        ...
        $("input[name=reenter_password]").change(function() {
            ...
            if ($("input[name=reenter_password]").hasClass('is-valid') && $("input[name=new_password2]")
                .hasClass('is-valid') && $("input[name=reenter_password]").val() == $(
        ...
        $('.check_order2').click(function() {
            console.log(window.location.origin + "/ProjectSem2/public/ajax/check-order/" + $(this).data(
                'order'));
            $.get(window.location.origin + "/ProjectSem2/public/ajax/check-order/" + $(this).data(
                'order'), function(data) {
                let dataJson = jQuery.parseJSON(data);
                console.log(dataJson);
                $('input[name=id_order2]').val(dataJson['id_order']);
                $("#receiver2").html(dataJson['receiver']);
                $("#address2").html(dataJson['address'] + "," + dataJson['ward'] + "," +
                    dataJson['district'] + "," + dataJson['province']);
                $('#instruction2').html(dataJson['instruction']);
                $("#phone2").html(dataJson['phone']);
                $('#email_order2').html(dataJson['email']);
                $("#payment_method2").html(dataJson['method']);
                ...
                total += parseInt(dataJson['shipping_fee']);
                $('#total_order2').html(total + " đ");
                $("#status_order2 option[value=" + dataJson['status'] + "]").attr("selected",
                    true);
        ...
        $('.clear_chat').click(function(e) {
            ...
            window.location.assign(window.location.origin + "/ProjectSem2/public/ajax/message/clear/" +
                code_gr);
        })
        //SHARE PRODUCT
